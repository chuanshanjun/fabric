...
// 配置资源
// 每个链在生成的时候都有它的配置，而这个配置就是被包裹在配置资源中的
...
func (ml *multiLedger) newChain(configtx *cb.Envelope) {
	// 生成账本资源对象
	ledgerResources := ml.newLedgerResources(configtx)
	// 将传进来的配置交易组装成一个区块,然后将区块append这个链上，也就是他的创世区块

...
type server struct {
	// 这两个成员变量是两个interface,里面封装了各自的handler方法
	// 是对grpc接口的服务封装，主要用于交易收集和区块扩散
	bh broadcast.Handler // 交易收集 所有客户端向order节点提交交易的时候，都会被这个变量处理
	dh deliver.Handler // 区块扩散 通过这个变量向其他区块的主节点，进行广播
}
...
func NewServer(ml multichain.Manager, signer crypto.LocalSigner) ab.AtomicBroadcastServer {
	// 实例化一个内部类
	s := &server{
		// 对broadcast和deliver进行了实例化，且deliver又进行了一次封装
...
// broadcast和deliver这两个接口在 protos.orderer.ab.proto中定义的AtomicBroadcast中
// 这就是server为什么要实现这两个方法的原因
